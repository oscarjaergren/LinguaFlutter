  ...
  group('PracticeCompletionScreen', () {
    ...
    testWidgets('should calculate and display accuracy', (tester) async {
      await tester.pumpWidget(
        buildTestWidget(correctCount: 8, incorrectCount: 2),
      );
    ...
    testWidgets('should display 100% accuracy when all correct', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestWidget(correctCount: 10, incorrectCount: 0),
      );

      expect(find.text('100%'), findsOneWidget);
    });

    testWidgets('should display 0% accuracy when all incorrect', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestWidget(correctCount: 0, incorrectCount: 10),
      );
    ...
    testWidgets('should display duration in minutes and seconds', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestWidget(duration: const Duration(minutes: 3, seconds: 45)),
      );

      expect(find.textContaining('3 min'), findsOneWidget);
    });

    testWidgets('should display duration in seconds only when under a minute', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestWidget(duration: const Duration(seconds: 45)),
      );
    ...
    testWidgets('should call onRestart when Practice Again button tapped', (
      tester,
    ) async {
      await tester.pumpWidget(buildTestWidget());
    ...
    testWidgets('should show check_circle icon for correct stats', (
      tester,
    ) async {
      await tester.pumpWidget(buildTestWidget());
