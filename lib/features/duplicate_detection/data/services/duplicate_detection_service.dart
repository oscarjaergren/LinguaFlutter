...
class DuplicateDetectionService {
  ...
  DuplicateDetectionService({this.config = DuplicateDetectionConfig.standard});
  ...
  DuplicateMatch? _checkExactMatch(CardModel card1, CardModel card2) {
    if (card1.frontText == card2.frontText &&
        card1.backText == card2.backText) {
  ...
  DuplicateMatch? _checkNormalizedWhitespaceMatch(
    CardModel card1,
    CardModel card2,
  ) {
  ...
  DuplicateMatch? _checkSameFrontDifferentBack(
    CardModel card1,
    CardModel card2,
  ) {
    final front1 = _normalizeWhitespace(card1.frontText);
  ...
  DuplicateMatch? _checkSameBackDifferentFront(
    CardModel card1,
    CardModel card2,
  ) {
    final back1 = _normalizeWhitespace(card1.backText);
    ...
    if (frontSimilarity >= config.fuzzyThreshold) {
      ...
      if (backSimilarity >= config.fuzzyThreshold) {
        ...
        return DuplicateMatch(
          ...
          reason:
              'Similar content (${(avgSimilarity * 100).toStringAsFixed(0)}% match)',
        );
      }
    }
    return null;
